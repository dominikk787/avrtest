 ...
 */
#if DHT_FLOAT == 1
int8_t dht_getdata(uint16_t *temperature, uint16_t *humidity) {
#elif DHT_FLOAT == 0
	...
	if ((uint8_t)(bits[0] + bits[1] + bits[2] + bits[3]) == bits[4]) {
		...
		*humidity = bits[0];
		...
		if(rawtemperature & 0x8000) {
			*temperature = (uint16_t)(rawtemperature & 0x7FFF) * -1;
		} else {
			*temperature = (uint16_t)(rawtemperature);
		}
		*humidity = (uint16_t)(rawhumidity);
		#endif
		return 0;
	}
 ...
 */
#if DHT_FLOAT == 1
int8_t dht_gettemperature(uint16_t *temperature) {
	uint16_t humidity = 0;
#elif DHT_FLOAT == 0
 ...
 */
#if DHT_FLOAT == 1
int8_t dht_gettemperaturehumidity(uint16_t *temperature, uint16_t *humidity) {
#elif DHT_FLOAT == 0
